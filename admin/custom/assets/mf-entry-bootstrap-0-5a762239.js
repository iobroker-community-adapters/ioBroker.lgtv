...
(async () => {
  const __mfHostInit = await __mfImport("./hostInit-CtuwwAC_.js");
  await __mfHostInit.__tla;
  const { initHost } = __mfHostInit;
  await initHost();
  const __mfPendingShares = await __mfImport("./pendingShares-sRXfmkm3.js").catch(() => undefined);
...
})().then(() => __mfImport("./index-CpwxNlH3.js"));
