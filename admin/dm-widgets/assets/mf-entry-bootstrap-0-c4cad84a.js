...
(async () => {
  const __mfHostInit = await __mfImport("./hostInit-Kh5g7tk0.js");
  await __mfHostInit.__tla;
  const { initHost } = __mfHostInit;
  await initHost();
  const __mfPendingShares = await __mfImport("./pendingShares-CkvnvW5g.js").catch(() => undefined);
...
})().then(() => __mfImport("./index-C9uLpd_T.js"));
