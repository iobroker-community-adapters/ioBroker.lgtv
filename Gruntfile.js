...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Sebastian Mundt';
    var email     = grunt.option('email')  || 'mail@sebastian-mundt.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new lgtv name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/lgtv.png')) {
            fs.renameSync(__dirname + '/admin/lgtv.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/lgtv.html')) {
            fs.renameSync(__dirname + '/widgets/lgtv.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/lgtv/js/lgtv.js')) {
            fs.renameSync(__dirname + '/widgets/lgtv/js/lgtv.js', __dirname + '/widgets/lgtv/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/lgtv')) {
            fs.renameSync(__dirname + '/widgets/lgtv',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Lgtv/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Lgtv'
                        },
                        {
                            match: /Sebastian Mundt/g,
                            replacement: author
                        },
                        {
                            match: /mail@sebastian-mundt.com/g,
                            replacement: email
                        }
                    ]
                },
