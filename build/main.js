...
class LgTv extends utils.Adapter {
    ...
    onStateChange(id, state) {
        ...
        this.log.debug(`State change "${id}" - VALUE: ${state.val}`);
        ...
        switch (id) {
            case 'states.popup':
                this.log.debug(`Sending popup message "${state.val}" to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.power':
                ...
                break;
            case 'states.mute':
                this.log.debug(`Sending mute ${state.val} command to WebOS TV: ${this.config.ip}`);
                ...
                break;
            case 'states.volume':
                this.log.debug(`Sending volume change ${state.val} command to WebOS TV: ${this.config.ip}`);
                this.oldVolume = this.volume;
                this.setVolume(Number(state.val));
                break;
            case 'states.volumeUp':
                this.log.debug(`Sending volumeUp ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.volumeDown':
                this.log.debug(`Sending volumeDown ${state.val} command to WebOS TV: ${this.config.ip}`);
                ...
                break;
            case 'states.channel':
                this.log.debug(`Sending switch to channel ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.channelUp':
                this.log.debug(`Sending channelUp ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.channelDown':
                this.log.debug(`Sending channelDown ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.mediaPlay':
                this.log.debug(`Sending mediaPlay ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.mediaPause':
                this.log.debug(`Sending mediaPause ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.openURL':
                ...
                this.log.debug(`Sending open ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.mediaStop':
                this.log.debug(`Sending mediaStop ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.mediaFastForward':
                this.log.debug(`Sending mediaFastForward ${state.val} command to WebOS TV: ${this.config.ip}`);
            ...
            case 'states.mediaRewind':
                this.log.debug(`Sending mediaRewind ${state.val} command to WebOS TV: ${this.config.ip}`);
                ...
                break;
            case 'states.3Dmode':
                this.log.debug(`Sending 3Dmode ${state.val} command to WebOS TV: ${this.config.ip}`);
                ...
                break;
            case 'states.launch':
                this.log.debug(`Sending launch command ${state.val} to WebOS TV: ${this.config.ip}`);
                this.launchApp(state.val);
                break;
            case 'states.input':
                this.log.debug(`Sending switch to input "${state.val}" command to WebOS TV: ${this.config.ip}`);
                ...
                break;
            case 'states.raw':
                this.log.debug(`Sending RAW command api "${state.val}" to WebOS TV: ${this.config.ip}`);
                try {
                    const obj = JSON.parse(String(state.val));
                    ...
                    this.log.error(`Parse error RAW command api - ${e}`);
                }
                break;
            case 'states.youtube': {
                ...
                this.sendCommand('ssap://system.launcher/launch', { id: 'youtube.leanback.v4', contentId: uri }, err => {
                    if (!err) {
            ...
            case 'states.soundOutput':
                this.sendCommand('ssap://com.webos.service.apiadapter/audio/changeSoundOutput', { output: state.val }, err => {
                    if (!err) {
                        void this.setState(id, state.val, true);
                    }
                });
    ...
    launchApp(val) {
        ...
        this.log.debug(`Opening app ${val} on WebOS TV: ${this.config.ip}`);
        this.sendCommand('ssap://system.launcher/launch', { id: val }, err => {
            if (!err) {
                void this.setState('states.launch', val, true);
            }
            else {
                this.log.debug(`Error opening app ${val} on WebOS TV: ${this.config.ip}`);
            }
        });
    }
    handleRemoteKey(id, state) {
    ...
    async connect(cb) {
        ...
        const lgtv = new LGTV({
            ...
            saveKey: (key, keyCb) => {
                (0, node_fs_1.writeFile)(this.keyFile, key, keyCb);
            },
    ...
    subscribeTv(lgtv) {
        lgtv.subscribe('ssap://audio/getVolume', (_err, res) => {
        ...
        lgtv.request('ssap://tv/getExternalInputList', {}, (err, res) => {
            if (!err && res?.devices) {
        ...
        lgtv.subscribe('ssap://tv/getCurrentChannel', (err, res) => this.applyCurrentChannel(err, res));
        lgtv.subscribe('ssap://com.webos.applicationManager/getForegroundAppInfo', (err, res) => {
            ...
            this.setTimeout(() => {
                ...
                if (this.healthInterval && !this.config.healthInterval) {
                    ...
                    this.log.info('detect poweroff event, polling not longer nessesary. if you have problems, check settings');
                }
        ...
        PICTURE_KEYS.forEach(key => {
            const cat = 'picture';
            lgtv.subscribe('ssap://settings/getSystemSettings', { category: cat, keys: [key] }, (err, res) => {
                if (err) {
                ...
                if (raw !== undefined && raw !== null) {
                    ...
                    this.log.debug(`getSystemSettings ${cat}.${key} no value: ${JSON.stringify(res).slice(0, 200)}`);
                }
            });
        });
        ...
        this.sendCommand('ssap://com.webos.service.update/getCurrentSWInformation', null, (err, val) => {
            if (err) {
                return;
            }
        ...
        this.sendCommand('ssap://system/getSystemInfo', null, (err, val) => {
            if (err) {
                return;
            }
    ...
    setPictureSetting(key, value) {
        ...
        if (PICTURE_BOOLEAN_KEYS.has(key)) {
            ...
            if (mapped === null) {
                this.log.warn(`set picture.${key}=${value} ignored — value is not a boolean`);
                return;
            }
            lunaValue = mapped;
        ...
        this.sendCommand('ssap://system.notifications/createAlert', {
            title: ' ',
            message: ' ',
            modal: true,
            type: 'confirm',
            ...
            if (err) {
                this.log.warn(`set picture.${key}=${lunaValue} failed: ${err}`);
                return;
            }
            this.log.debug(`set picture.${key}=${lunaValue}: ${JSON.stringify(val)}`);
    ...
    checkReconnectWatchdog() {
        ...
        (0, probe_1.probeTcpReachable)(this.config.ip, WATCHDOG_PROBE_PORT, WATCHDOG_PROBE_TIMEOUT_MS)
            .then(reachable => {
            if (!reachable) {
                ...
                this.lastConnectingAt = Date.now();
                this.log.debug(`[WATCHDOG] TV ${this.config.ip}:${WATCHDOG_PROBE_PORT} not reachable — skipping recreate`);
                return;
            }
            this.log.warn(`[WATCHDOG] No reconnect attempt for ${Math.round(idleMs / 1000)}s while disconnected — recreating LGTV instance`);
            try {
                void this.lgtv?.disconnect(); // the returned promise never rejects
            }
            catch (err) {
                this.log.debug(`[WATCHDOG] disconnect failed: ${err}`);
            }
            ...
            .catch(err => {
            this.log.debug(`[WATCHDOG] probe failed: ${err}`);
        })
            .finally(() => {
            this.watchdogProbeInFlight = false;
        });
    }
    checkCurApp(powerOff) {
        if (powerOff) {
            this.curApp = '';
        }
        ...
        if (this.curApp === 'com.webos.app.livetv') {
            this.setTimeout(() => {
                this.lgtv?.subscribe('ssap://tv/getCurrentChannel', (err, res) => this.applyCurrentChannel(err, res));
            }, 3000);
        }
        ...
        this.setStateChanged('states.on', isTVon, true, (_err, _stateId, notChanged) => {
            ...
            this.renewTimeout = this.setTimeout(() => {
                ...
                if (this.healthInterval !== false) {
                    this.healthInterval = this.setInterval(() => this.sendCommand('ssap://com.webos.service.tv.time/getCurrentTime', null, err => {
    ...
    sendCommand(cmd, options = null, cb) {
        if (this.isConnect) {
            this.sendPacket(cmd, options, (error, response) => cb?.(error, response));
        }
    }
    sendPacket(cmd, options, cb) {
        ...
        if (~cmd.indexOf('ssap:') || ~cmd.indexOf('com.')) {
            lgtv.request(cmd, options ?? {}, (error, response) => {
                if (error) {
            ...
            lgtv.getSocket('ssap://com.webos.service.networkinput/getPointerInputSocket', (err, sock) => {
                ...
                try {
                    ...
                    this.log.debug(`ERROR sending WebOS remote input command: ${sendError}`);
                    cb?.(sendError);
                }
            });
        }
    ...
    wakeTv() {
        this.getState(`${this.namespace}.states.mac`, (err, macState) => {
            ...
            try {
                ...
                (0, wol_1.wake)(mac, wakeOptions, wakeError => {
                    if (wakeError) {
                ...
                this.log.error(`Cannot wake TV: invalid MAC address "${mac}": ${e}`);
            }
        });
    }
    setVolume(val) {
    ...
    onReady() {
        ...
        const dir = (0, node_path_1.join)(utils.getAbsoluteDefaultDataDir(), this.namespace.replace('.', '_'));
        this.keyFile = (0, node_path_1.join)(dir, this.keyFile);
        ...
        (0, node_fs_1.readFile)(this.keyFile, (err, data) => {
            if (err) {
                (0, node_fs_1.writeFile)(this.keyFile, '', writeErr => {
                    if (writeErr) {
...
if (require.main !== module) {
    ...
    module.exports = (options) => new LgTv(options);
}
else {
