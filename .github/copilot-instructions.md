...
## Adapter-Specific Context: LG WebOS SmartTV

**Adapter Name:** lgtv
**Primary Function:** Controls and monitors LG WebOS Smart TVs via WebSocket connection
**Key Dependencies:**
- `lgtv2`: WebOS TV communication library for connection and command sending
- `websocket`: WebSocket client for persistent TV communication
- `wol`: Wake-on-LAN functionality to power on TVs remotely

**Configuration Requirements:**
- TV IP address (required for connection)
- Optional polling interval for TVs that disconnect without proper notification
- Optional Wake-on-LAN settings (MAC address, broadcast address, unicast support)
- Health check interval configuration (minimum 5 seconds)

**Device Interaction Patterns:**
- TV pairing process: First connection triggers on-screen pairing prompt
- WebSocket connection management with automatic reconnection
- State synchronization for TV power, volume, channel, input sources
- Command execution for TV control (power, volume, channel change, app launching)
- Polling fallback for TVs that don't properly report disconnection

**Key Integration Points:**
- WebOS TV command API (ssap:// protocol)
- TV input switching (HDMI1-3, AV1, Component, Scart)
- App launching (Netflix, YouTube, Amazon Prime, etc.)
- Mouse/pointer control and drag operations
- Raw JSON command support for advanced TV features
...
### LGTV Adapter Specific Testing Considerations

**Hardware Testing Limitations:**
- LG WebOS TVs are physical devices that cannot be easily mocked
- Integration tests should focus on adapter initialization and error handling
- Use mock WebSocket connections for testing connection logic
- Test TV pairing workflow with simulated responses
- Verify state creation without requiring actual TV hardware

**Mock TV Testing Patterns:**
```javascript
// Example: Test TV connection logic without hardware
it('should handle TV connection timeout gracefully', function () {
    return new Promise(async (resolve, reject) => {
        try {
            harness = getHarness();
            
            const obj = await new Promise((res, rej) => {
                harness.objects.getObject('system.adapter.lgtv.0', (err, o) => {
                    if (err) return rej(err);
                    res(o);
                });
            });

            // Configure with non-existent IP to test timeout handling
            Object.assign(obj.native, {
                ip: '192.168.1.999', // Invalid IP to trigger timeout
                healthInterval: 5000
            });
            ...
            // Wait for connection attempt and timeout
            await new Promise(res => setTimeout(res, 10000));
            
            const connectionState = await new Promise((res, rej) => {
                harness.states.getState('lgtv.0.info.connection', (err, state) => {
                    if (err) return rej(err);
                    res(state);
                });
            });
            ...
            if (!connectionState || connectionState.val === false) {
                console.log('✅ Adapter properly handled connection timeout');
                resolve(true);
            } else {
                reject(new Error('Adapter should have failed to connect to invalid IP'));
            }
